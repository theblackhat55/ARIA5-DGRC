-- ARIA5.1 Enhanced Dynamic Risk Engine Schema
...
CREATE TABLE IF NOT EXISTS service_indices (
  ...
  -- Composite scoring
  composite_criticality REAL GENERATED ALWAYS AS (
    0.35 * svi + 0.35 * sei + 0.20 * bci + 0.10 * eri
  ) STORED,
...
CREATE TABLE IF NOT EXISTS vulnerabilities (
  ...
  age_days INTEGER GENERATED ALWAYS AS (
    CAST((julianday('now') - julianday(discovered_date)) AS INTEGER)
  ) STORED,
...
CREATE TABLE IF NOT EXISTS risk_score_history (
  ...
  old_score REAL,
  new_score REAL,
  score_delta REAL GENERATED ALWAYS AS (new_score - old_score) STORED,
  old_risk_level TEXT,
  new_risk_level TEXT,
...
-- =============================================================================
-- ENHANCED EXISTING TABLES - Add composite scoring fields
-- =============================================================================

-- Add normalized scoring fields to existing risks table (additive, no breaking changes)
ALTER TABLE risks ADD COLUMN risk_score_composite REAL DEFAULT 0 
  CHECK (risk_score_composite >= 0 AND risk_score_composite <= 100);
ALTER TABLE risks ADD COLUMN likelihood_0_100 REAL DEFAULT 0 
  CHECK (likelihood_0_100 >= 0 AND likelihood_0_100 <= 100);
ALTER TABLE risks ADD COLUMN impact_0_100 REAL DEFAULT 0 
  CHECK (impact_0_100 >= 0 AND impact_0_100 <= 100);
ALTER TABLE risks ADD COLUMN controls_discount REAL DEFAULT 0 
  CHECK (controls_discount >= 0 AND controls_discount <= 30);
ALTER TABLE risks ADD COLUMN final_score REAL DEFAULT 0 
  CHECK (final_score >= 0 AND final_score <= 1);
ALTER TABLE risks ADD COLUMN score_explanation TEXT; -- Top contributing factors
ALTER TABLE risks ADD COLUMN dedupe_key TEXT; -- {tenant}:{service_id}:{category}:{signal_fingerprint}:{day}
ALTER TABLE risks ADD COLUMN merged_from_risk_ids TEXT; -- JSON array of merged risk IDs

-- Add service criticality tracking
ALTER TABLE services ADD COLUMN current_criticality REAL DEFAULT 0 
  CHECK (current_criticality >= 0 AND current_criticality <= 100);
...
ALTER TABLE services ADD COLUMN criticality_trend TEXT DEFAULT 'stable' 
  CHECK (criticality_trend IN ('decreasing', 'stable', 'increasing'));
...
CREATE INDEX IF NOT EXISTS idx_risk_score_history_score_delta ON risk_score_history(ABS(score_delta) DESC);
...
-- Enhanced risks table indexes
CREATE INDEX IF NOT EXISTS idx_risks_composite_score ON risks(risk_score_composite DESC);
CREATE INDEX IF NOT EXISTS idx_risks_final_score ON risks(final_score DESC);
CREATE INDEX IF NOT EXISTS idx_risks_dedupe_key ON risks(dedupe_key);
